%% generalised errorbars plotting function for 4ch decoding timeseries by average ces. 
% inputs:
% acc_source_4ch_100/200/400/600ms_epochs_df/ms(_g3)(_2/5/10ds).mat
...
for ds = 1:4
    for x = 1:2
        for g = 1:2
            for q = 1:4
                epoch = epochs(q);
                load(['acc_source_4ch_' num2str(epoch) 'ms_epochs' dslabel{ds} glabel{g} '_p3' xlabel{x}]);
                Ydata = mean(Acc_4ch_source(:,:),[2]);
                prc = prctile(Acc_4ch_source, [2.5 97.5], 2);
...
% Create figure
figure1 = figure;
savetitle1 = ['source_4ch_ces' num2str(epoch) 'ms' ds g '_p3' x];
% Create axes
...
% Create title
title(['Decoding of 31/32 by 4ch source IIS (' num2str(epoch) 'ms) '...
